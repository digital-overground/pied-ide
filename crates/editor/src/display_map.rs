...
fn mask_chunks<'a>(chunks: impl Iterator<Item = Chunk<'a>>) -> impl Iterator<Item = Chunk<'a>> {
    chunks.flat_map(|chunk| {
        ...
        text.split_inclusive('\n').flat_map(move |segment| {
            ...
            // Most chunks fit `BULLETS` (their bitmasks cap them at 128
            // bytes), but synthetic chunks such as soft-wrap indentation (up to
            // `LineWrapper::MAX_INDENT` spaces) can be longer, so emit the
            // bullets in `BULLETS`-sized pieces.
...
/// coordinates.
///
/// On snapshots without soft wraps, rows longer than `MAX_LINE_LEN` are never
/// shaped (shaping is O(row length)); their columns are positioned on the
/// monospace grid the renderer uses for such rows, so columnar-selection and
/// pointer math stay bounded on huge lines.
...
impl RowLayout<'_> {
    ...
    pub fn x_for_index(&self, index: usize) -> Pixels {
        match self {
            ...
            Self::Grid {
                cell_width, len, ..
            } => *cell_width * index.min(*len as usize) as f32,
        }
    }
...
impl DisplaySnapshot {
    ...
    pub fn layout_row(
        ...
        // With soft wraps present rows are bounded by the wrap width, so
        // shaping stays cheap and stays consistent with the painted glyphs.
...
pub mod tests {
    ...
    async fn test_layout_row_shapes_long_rows_when_soft_wrapped(cx: &mut gpui::TestAppContext) {
        ...
        cx.update_window(window, |_, window, cx| {
            ...
            // Wrapped rows are bounded by the wrap width, so they are shaped
            // even when they exceed `MAX_LINE_LEN`; the monospace grid is only
            // used on snapshots without soft wraps.
            assert!(matches!(
